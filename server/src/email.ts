...
const gmailReady = Boolean(config.GMAIL_CLIENT_ID && config.GMAIL_CLIENT_SECRET && config.GMAIL_REFRESH_TOKEN && config.GMAIL_SENDER)

const oauth2Client = new google.auth.OAuth2(config.GMAIL_CLIENT_ID, config.GMAIL_CLIENT_SECRET)
if (gmailReady) oauth2Client.setCredentials({ refresh_token: config.GMAIL_REFRESH_TOKEN })
...
export async function sendPasswordResetEmail(to: string, code: string): Promise<void> {
  if (!gmailReady) {
    console.log(`[email] Gmail não configurado — código de reset para ${to}: ${code}`)
    return
  }
