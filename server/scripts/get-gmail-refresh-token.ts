...
if (!clientId || !clientSecret) {
  console.error('Defina GMAIL_CLIENT_ID e GMAIL_CLIENT_SECRET no .env antes de rodar este script.')
  process.exit(1)
}
...
const code: string = await new Promise((resolve, reject) => {
  const server = http.createServer((req, res) => {
    ...
    res.end(error ? '<p>Autorização negada. Pode fechar esta aba.</p>' : '<p>Autorizado! Pode fechar esta aba e voltar ao terminal.</p>')
    server.close()
