...
function Modal({ title, onClose, children, wide }: { title: string; onClose: () => void; children: React.ReactNode; wide?: boolean }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className={`bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden ${wide ? 'w-full max-w-2xl' : 'w-full max-w-md'}`}
...
function KpiCard({ label, value, sub, delta, color, icon }: { label: string; value: string | number; sub?: string; delta?: number; color: string; icon: React.ReactNode }) {
  return (
    <div className="bg-white rounded-2xl p-5" style={{ border: '1.5px solid #E2E8F0', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs font-medium text-slate-500 mb-1">{label}</p>
          <p className="text-2xl font-bold text-slate-900" style={{ fontFamily: "'DM Sans', sans-serif", color }}>
      ...
      {delta !== undefined && (
        <div className="mt-2 flex items-center gap-1 text-xs">
          {delta >= 0 ? (
            <><ArrowUp size={11} className="text-emerald-500" /><span className="text-emerald-600 font-medium">+{delta}%</span></>
          ) : (
            <><ArrowDown size={11} className="text-red-500" /><span className="text-red-600 font-medium">{delta}%</span></>
          )}
...
const channelDist = [
  ...
  { name: 'Site', value: 10, color: '#8B5CF6' },
]
    ...
    <div className="h-full overflow-auto p-5">
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          <KpiCard label="Alcance Instagram" value={igReach} sub="soma posts publicados" delta={14} color="#EC4899" icon={<TrendingUp size={18} />} />
          <KpiCard label="Impressões LinkedIn" value={liImpressions} sub="soma posts publicados" delta={8.2} color="#3B82F6" icon={<Globe size={18} />} />
          <KpiCard label="Taxa de Engajamento IG" value={`${igEngRate}%`} sub="eng / alcance × 100" delta={0.3} color="#10B981" icon={<BarChart2 size={18} />} />
          <KpiCard label="MQLs este mês" value={mql.monthlyMQLs} sub={`Taxa MQL→SQL: ${mql.mqlToSQLRate}%`} delta={12} color="#6366F1" icon={<Users size={18} />} />
        </div>

        {/* Custom metrics */}
        {metrics.length > 0 && (
          <div>
            <h3 className="text-sm font-semibold text-slate-700 mb-3" style={{ fontFamily: "'DM Sans', sans-serif" }}>Métricas personalizadas</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {metrics.map((m) => (
                <div key={m.id} className="bg-white rounded-xl p-4" style={{ border: '1.5px solid #E2E8F0' }}>
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <p className="text-xs text-slate-500 leading-snug flex-1">{m.name}</p>
                    <div className="w-2 h-2 rounded-full flex-shrink-0 mt-0.5" style={{ background: m.color }} />
                  </div>
                  <p className="text-xl font-bold" style={{ color: m.color, fontFamily: "'DM Sans', sans-serif" }}>
                    {typeof m.value === 'number' ? m.value.toLocaleString('pt-BR') : m.value}
                    <span className="text-sm font-normal ml-1 text-slate-400">{m.unit}</span>
                  </p>
                  <p className="text-xs text-slate-400 mt-1 leading-snug">{m.formula}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5" style={{ border: '1.5px solid #E2E8F0' }}>
          <div className="flex items-center gap-2 mb-4">
            <h3 className="text-sm font-semibold text-slate-800" style={{ fontFamily: "'DM Sans', sans-serif" }}>
              Alcance semanal — calculado automaticamente pelos posts
            </h3>
            <div className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full" style={{ background: '#EEF2FF', color: '#4F46E5' }}>
              <Check size={10} /> auto
            </div>
          </div>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={weeklyReach} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="Instagram" stroke="#EC4899" strokeWidth={2} fill="url(#igGrad)" />
              <Area type="monotone" dataKey="LinkedIn" stroke="#3B82F6" strokeWidth={2} fill="url(#liGrad)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white rounded-2xl p-5" style={{ border: '1.5px solid #E2E8F0' }}>
            <h3 className="text-sm font-semibold text-slate-800 mb-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>Mix de canais</h3>
            <ResponsiveContainer width="100%" height={140}>
              <PieChart>
                ...
                <Tooltip formatter={(v) => `${v}%`} contentStyle={{ fontSize: 12, border: '1px solid #E2E8F0', borderRadius: 8 }} />
              </PieChart>
            </ResponsiveContainer>
            <div className="space-y-1.5 mt-2">
              {channelDist.map((c) => (
                <div key={c.name} className="flex items-center justify-between text-xs">
                  ...
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", color: '#64748B' }}>{c.value}%</span>
                </div>
              ))}
            </div>
          </div>

          <div className="md:col-span-2 bg-white rounded-2xl p-5" style={{ border: '1.5px solid #E2E8F0' }}>
            <h3 className="text-sm font-semibold text-slate-800 mb-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>Alcance por post (julho)</h3>
...
const METRIC_COLORS = ['#6366F1', '#EC4899', '#3B82F6', '#10B981', '#F59E0B', '#8B5CF6', '#EF4444', '#14B8A6']
...
function MetricModal({ initial, onSave, onClose }: { initial?: CustomMetric; onSave: (m: CustomMetric) => void; onClose: () => void }) {
  const [form, setForm] = useState<MetricForm>({
    ...
    channel: initial?.channel ?? '', color: initial?.color ?? '#6366F1',
  })
      ...
      <div className="px-6 py-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="block text-xs font-medium text-slate-600 mb-1">Nome da métrica *</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Valor atual</label>
          ...
          <div className="col-span-2">
            <label className="block text-xs font-medium text-slate-600 mb-1">Regra de cálculo / fórmula</label>
            <textarea value={form.formula} onChange={(e) => setForm((f) => ({ ...f, formula: e.target.value }))} rows={3}
              ...
              className="w-full text-sm px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:border-indigo-400 resize-none" />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Canal (opcional)</label>
            <select value={form.channel} onChange={(e) => setForm((f) => ({ ...f, channel: e.target.value as ChannelType | '' }))}
              className="w-full text-sm px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:border-indigo-400 bg-white">
    ...
    <div className="h-full overflow-auto p-5">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-5">
          <h3 className="font-semibold text-slate-800" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div className="space-y-3">
          {metrics.map((m) => (
            <div key={m.id} className="bg-white rounded-xl overflow-hidden group"
                  ...
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <p className="text-sm font-semibold text-slate-800" style={{ fontFamily: "'DM Sans', sans-serif" }}>{m.name}</p>
                        {m.channel && (
                          <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: '#F1F5F9', color: '#64748B' }}>{CH_LABELS[m.channel]}</span>
                        )}
                      </div>
                      <p className="text-xs text-slate-500">{m.formula}</p>
                    </div>
                  </div>
                    ...
                    {editingId === m.id ? (
                      <div className="flex items-center gap-1">
                        <input type="number" value={editVal} onChange={(e) => setEditVal(e.target.value)}
                          ...
                          className="w-20 text-sm px-2 py-1 rounded border border-indigo-300 focus:outline-none text-center"
                          style={{ fontFamily: "'JetBrains Mono', monospace" }} autoFocus />
                        <button onClick={() => commitValueEdit(m.id)} className="text-emerald-500 hover:text-emerald-700"><Check size={14} /></button>
                      ...
                      <button onClick={() => { setEditingId(m.id); setEditVal(String(m.value)) }}
                        ...
                        {m.value.toLocaleString('pt-BR')} <span className="text-sm font-normal text-slate-400">{m.unit}</span>
                      </button>
                    )}
                    <button onClick={() => setModal({ metric: m })} className="p-1.5 rounded-lg text-slate-300 hover:text-indigo-500 hover:bg-indigo-50 opacity-0 group-hover:opacity-100 transition-all">
                      <Edit2 size={13} />
                    </button>
                    <button onClick={() => setDeleteId(m.id)} className="p-1.5 rounded-lg text-slate-300 hover:text-red-400 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>
          ...
          {metrics.length === 0 && (
            <div className="text-center py-12 text-slate-400">Nenhuma métrica. Clique em "Nova métrica" para começar.</div>
          )}
        </div>
        ...
        {deleteId !== null && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setDeleteId(null)}>
            <div className="bg-white rounded-2xl p-6 shadow-2xl max-w-sm w-full mx-4" onClick={(e) => e.stopPropagation()}>
              <p className="font-semibold text-slate-800 mb-1" style={{ fontFamily: "'DM Sans', sans-serif" }}>Apagar métrica?</p>
              <p className="text-sm text-slate-500 mb-4">Esta ação não pode ser desfeita.</p>
              <div className="flex gap-2">
                <button onClick={() => deleteMetric(deleteId)} className="px-4 py-2 rounded-xl text-sm font-medium text-white bg-red-500">Apagar</button>
  ...
  return (
    <div className="h-full overflow-auto p-5">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-base font-semibold text-slate-800" style={{ fontFamily: "'DM Sans', sans-serif" }}>Definição do MQL Ideal</h2>
            <p className="text-sm text-slate-500 mt-0.5">Critérios de qualificação e conversão</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
          {[
            { label: 'MQLs este mês', key: 'monthlyMQLs' as keyof MQLState, color: '#6366F1', suffix: '' },
            { label: 'Taxa MQL → SQL', key: 'mqlToSQLRate' as keyof MQLState, color: '#10B981', suffix: '%' },
            { label: 'Score mínimo', key: 'score' as keyof MQLState, color: '#F59E0B', suffix: '/100' },
          ].map((kpi) => (
            <div key={kpi.key} className="bg-white rounded-xl p-4" style={{ border: '1.5px solid #E2E8F0' }}>
              ...
              {editMode ? (
                <input type="number" value={mql[kpi.key] as number}
                  ...
                  className="text-2xl font-bold w-full focus:outline-none bg-transparent border-b-2 border-indigo-300"
                  style={{ color: kpi.color, fontFamily: "'DM Sans', sans-serif" }} />
              ) : (
                <div className="text-2xl font-bold" style={{ color: kpi.color, fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div className="grid grid-cols-2 gap-4">
          {/* Job titles */}
          <div className="bg-white rounded-2xl p-5" style={{ border: '1.5px solid #E2E8F0' }}>
            <h3 className="text-sm font-semibold text-slate-800 mb-3" style={{ fontFamily: "'DM Sans', sans-serif" }}>Cargos-alvo</h3>
            <div className="flex flex-wrap gap-2">
              {mql.jobTitles.map((t, i) => (
                <span key={i} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full font-medium" style={{ background: '#EEF2FF', color: '#4338CA' }}>
                  {t}
            ...
            {editMode && (
              <div className="flex gap-2 mt-3">
                <input value={newTitle} onChange={(e) => setNewTitle(e.target.value)}
                  placeholder="Novo cargo..." className="flex-1 text-xs px-2 py-1.5 rounded-lg border border-slate-200 focus:outline-none focus:border-indigo-400"
                  onKeyDown={(e) => { if (e.key === 'Enter') { addItem('jobTitles', newTitle); setNewTitle('') } }} />
                <button onClick={() => { addItem('jobTitles', newTitle); setNewTitle('') }} className="text-xs px-2 py-1.5 rounded-lg bg-indigo-100 text-indigo-700"><Plus size={12} /></button>
              </div>
            )}
          </div>
          ...
          {/* Industries */}
          <div className="bg-white rounded-2xl p-5" style={{ border: '1.5px solid #E2E8F0' }}>
            <h3 className="text-sm font-semibold text-slate-800 mb-3" style={{ fontFamily: "'DM Sans', sans-serif" }}>Segmentos</h3>
            <div className="flex flex-wrap gap-2">
              {mql.industries.map((ind, i) => (
                <span key={i} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full font-medium" style={{ background: '#F0FDF4', color: '#15803D' }}>
                  {ind}
            ...
            {editMode && (
              <div className="flex gap-2 mt-3">
                <input value={newIndustry} onChange={(e) => setNewIndustry(e.target.value)}
                  placeholder="Nova indústria..." className="flex-1 text-xs px-2 py-1.5 rounded-lg border border-slate-200 focus:outline-none focus:border-indigo-400"
                  onKeyDown={(e) => { if (e.key === 'Enter') { addItem('industries', newIndustry); setNewIndustry('') } }} />
                <button onClick={() => { addItem('industries', newIndustry); setNewIndustry('') }} className="text-xs px-2 py-1.5 rounded-lg bg-emerald-100 text-emerald-700"><Plus size={12} /></button>
              </div>
            )}
          </div>
          ...
          {/* Company size */}
          <div className="bg-white rounded-2xl p-5" style={{ border: '1.5px solid #E2E8F0' }}>
            <h3 className="text-sm font-semibold text-slate-800 mb-3" style={{ fontFamily: "'DM Sans', sans-serif" }}>Tamanho da empresa</h3>
            {editMode ? (
              <input value={mql.companySize} onChange={(e) => setMql((m) => ({ ...m, companySize: e.target.value }))}
                className="w-full text-sm px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:border-indigo-400" />
            ) : (
              <span className="text-sm px-3 py-1.5 rounded-full font-medium inline-block" style={{ background: '#FFF7ED', color: '#C2410C' }}>{mql.companySize}</span>
            )}
          </div>

          {/* Behaviors */}
          <div className="bg-white rounded-2xl p-5" style={{ border: '1.5px solid #E2E8F0' }}>
            <h3 className="text-sm font-semibold text-slate-800 mb-3" style={{ fontFamily: "'DM Sans', sans-serif" }}>Comportamentos qualificadores</h3>
            <div className="space-y-1.5">
              {mql.behaviors.map((beh, i) => (
                <div key={i} className="flex items-center justify-between py-1.5 px-2 rounded-lg" style={{ background: '#F8FAFC' }}>
                  <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-400 flex-shrink-0" />
                    <span className="text-xs text-slate-700">{beh}</span>
                  </div>
                  {editMode && <button onClick={() => removeItem('behaviors', i)} className="text-slate-400 hover:text-red-400"><X size={13} /></button>}
                </div>
              ))}
            </div>
            {editMode && (
              <div className="flex gap-2 mt-2">
                <input value={newBehavior} onChange={(e) => setNewBehavior(e.target.value)}
                  placeholder="Novo comportamento..." className="flex-1 text-xs px-2 py-1.5 rounded-lg border border-slate-200 focus:outline-none focus:border-indigo-400"
                  onKeyDown={(e) => { if (e.key === 'Enter') { addItem('behaviors', newBehavior); setNewBehavior('') } }} />
                <button onClick={() => { addItem('behaviors', newBehavior); setNewBehavior('') }} className="text-xs px-2 py-1.5 rounded-lg bg-indigo-100 text-indigo-700"><Plus size={12} /></button>
              </div>
            )}
          </div>
        </div>
    ...
    <div className="flex flex-col h-full">
      <header className="bg-white flex-shrink-0" style={{ borderBottom: '1.5px solid #E2E8F0' }}>
        <div className="px-4 md:px-6 pt-4 md:pt-5 pb-0">
          <h1 className="text-lg md:text-xl font-semibold text-slate-900 leading-tight" style={{ fontFamily: "'DM Sans', sans-serif" }}>Métricas</h1>
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">Dashboard consolidado · Julho 2026</p>
        </div>
